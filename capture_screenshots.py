...
def _capture_all():
    with sync_playwright() as p:
        ...
        # Use the same names (and counts) as voting-app/demo_names.py's
        # FALLBACK_*_CANDIDATES so the phone-ballot screenshots match the
        # paper ballot generated by seed_demo.
        print("08. Add Elder office")
        page.fill('#office_name', 'Elder')
        page.fill('#vacancies', '3')
        for name in ['Henry Brouwerhof', 'James Groenevelt',
                     'William de Kempenaar', 'Neil ten Heuvel',
                     'Gary van Dijkstra', 'Peter van Rijksen']:
        ...
        for name in ['Fred Bosveldt', 'Ryan Mulderhoek',
                     'Derek van Leeuwenburg', 'Andrew Visserman']:
            add_candidate_tag(page, name)
        page.click('#add-office-form button[type="submit"]')
        page.wait_for_load_state("networkidle")
        ...
        if not codes:
            ...
            # Ballot order: 6 Elder candidates (0..5), then 4 Deacon
            # candidates (6..9). Tick 3 Elders + 2 Deacons.
            if n >= 10:
            ...
            for code in codes[1:12]:
                ...
                n = cbs.count()
                if n >= 10:
                    # 3 Elders out of indices 0..5
                    for idx in random.sample(range(6), 3):
                        cbs.nth(idx).check()
                    # 2 Deacons out of indices 6..9
                    for idx in random.sample(range(6, 10), 2):
                        cbs.nth(idx).check()
                    vp.click('button[type="submit"]')
                    vp.wait_for_load_state("networkidle")
                    cast += 1
