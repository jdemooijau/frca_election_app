...
def test_seed_demo_creates_default_codes(tmp_path):
    import sqlite3
    db_path = str(tmp_path / "test.db")
...
def test_seed_demo_presets_round1_attendance(tmp_path):
    import sqlite3
    db_path = str(tmp_path / "test.db")
    _run_seed_script("YES\n", db_path)
    conn = sqlite3.connect(db_path)
    row = conn.execute(
        "SELECT participants FROM round_counts WHERE round_number = 1"
    ).fetchone()
    conn.close()
    assert row is not None
    assert row[0] == 110
