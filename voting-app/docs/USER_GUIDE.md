...
Submitting the new-election form lands on the wizard sidebar at the
**Offices & Candidates** step. Add each office (Elder, Deacon) with:
...
Click **Codes & printing** in the wizard sidebar. The app auto-generates
enough codes for every member across the maximum number of rounds, plus
...
The same **Codes & printing** step has a **Print & prepare** strip with:
...
The same step has a **Postal votes (early)** strip with **Enter Postal
Votes (Round 1)** and a **Postal Tally Helper**. Postal votes count
only in Round 1. They are added to the Round 1 totals before
thresholds are calculated. There is also a last-chance entry on the
**Attendance & postal** step on the day itself.
...
On election day the wizard sidebar shifts focus to the **Round** group.
First click **Attendance & postal** and enter the **Brothers Present**
count from the register, then click Save. Without it the Article 6b
threshold cannot be calculated. (The paper-ballot count is entered
later, on the Count & tally step.)

Then click **Welcome & Rules** in the sidebar. This step contains:

- The projector phase stepper (Welcome -> Election Rules)
- An **Open Display** link to launch the projector view in a new tab
- A **Next: Election Rules ->** advance button
- A **Next: Voting ->** link that takes you to the Voting step

![Welcome & Rules step](../../screenshots/11_manage_before_voting.png)
...
3. **Next: Voting ->** - takes you to the Voting step where you click
   **Open Round 1** to open voting and switch the projector to the
   live ballot view
...
The **Voting** step shows live ballot counts while voting is open:

![Live tally](../../screenshots/18_manage_votes_live.png)

- Top stats: ballots received, brothers participating, still to vote
- A live per-office tally with green tick or red cross marks against
  Articles 6a and 6b for each candidate
...
- A **View voter audit log** link for diagnostics
...
On the Voting step click **Close Round N**. The sidebar advances to
**Count & tally**. Closing voting no longer auto-reveals the tally on
the projector - that is a separate, deliberate action (Step 16 below).

![Closed round](../../screenshots/19_manage_results_closed.png)
...
Have at least two brothers count the paper ballots together. On the
**Count & tally** step, enter the **Paper Ballots Received** total and
click Save. (Per-candidate paper tallies are entered separately in the
next step.)

### 15. Enter per-candidate paper votes

Click **Enter Paper Votes (per candidate)** on the Count & tally step.
Enter the count for each candidate from the counter sheet and save.

### 16. Show results on the projector

Click **Show Results on Projector** on the Count & tally step to reveal
the per-candidate totals to the congregation.
...
Click **Decide** in the sidebar. The step offers two choices:
...
If you advance to a new round, the sidebar returns to **Welcome &
Rules** for the new round and you walk the projector through Welcome
-> Rules -> Voting again. The same code batch is reused, no
re-printing needed.

### 18. Final results

The **Final results** step (under the **Finish** group in the sidebar)
becomes active when every vacancy is filled, or when you press **Show
Final Results** explicitly. It contains:
...
The **Voting** step has a **View voter audit log** link. This shows
every voter-route hit with a colour-coded result pill (accepted,
rejected_invalid, rejected_used, vote_submitted, etc.), along with the
timestamp, IP, user-agent, and the plaintext code.
